...
class App < Roda
  ...
  plugin :error_handler do |e|
    if e.instance_of?(Exceptions::InvalidParamsError)
      ...
      response.status = 401
    elsif e.instance_of?(Exceptions::InvalidPermissions)
      response.status = 401
    ...
    r.on('api') do
      r.on('v1') do
        ...
        r.on('movies') do
          ...
          r.on(:uuid) do |id|
            movie = Movie.with_average_rating.with_pk!(id)

            r.get do
              omdb_data = Omdb::Api.by_id(omdb_id: movie.omdb_id)

              MovieSerializer.new(movie: movie, omdb_data: omdb_data).render
            end

            r.post('rate') do
              rate_params = RateParams.new.permit!(r.params)

              Rating.update_or_create(movie: movie, user: current_user) do |rating|
                rating.value = rate_params[:value]
              end
            ...
            r.put do
              raise Exceptions::InvalidPermissions unless current_user.admin

              movie_params = MovieParams.new.permit!(r.params)

              movie.update(movie_params)

              MovieSerializer.new(movie: movie).render
            end
          end

          r.get do
            movies = Movie.with_average_rating.all

            MoviesSerializer.new(movies: movies).render
          end
        end
      end
    end
